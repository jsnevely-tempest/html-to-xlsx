...
function addRow (sheet, row, context) {
  ...
  for (let cIdx = 0; cIdx < row.length; cIdx++) {
    ...
    if (previousCell != null && previousCell.rowSpan !== rowSpan) {
      ...
      const start = context.currentRowInFile + 1
      const end = start + (rowSpan - 1);
      const usedCellRowVals = Object.entries(usedCells).map(c => c[0].split(',').map(i => parseInt(i))).reduce((a, c) => {
        let [cur_row, cur_cel] = c;
        cur_row--;
        if(a[cur_row] == null) a[cur_row] = [cur_cel]
        else a[cur_row].push(cur_cel)
        return a;
      }, {});
      for (let r = start; r < end; r++) {
        const cellOffsetsInRow = currentCellOffsetsPerRow[r]
        if(cellOffsetsInRow.length > 1) {
          let rowMaxAvailableCell = usedCellRowVals[r].sort().reduce((a, c) => {
            if(c - 1 !== a) return a;
            return c > a ? c : a;
          }, 1)
          if((cellOffsetsInRow[0].startCell + cellOffsetsInRow[0].offset) <= endCell) {
            cellOffsetsInRow[0].startCell = rowMaxAvailableCell + 1;
            cellOffsetsInRow[0].offset = 0
          }
        }
      }
    }
